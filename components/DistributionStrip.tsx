...
export default function DistributionStrip({ dist }: { dist: Distribution }) {
  ...
  const hereH = maxCount ? (dist.bins[dist.binIndex].count / maxCount) * (VB_H - 3) : 0;
    ...
    <div className="border-t border-line py-5 first:border-t-0 first:pt-0">
      ...
      <div className="mt-3">
        ...
        <svg viewBox={`0 0 ${VB_W} ${VB_H}`} width="100%" height={VB_H}
          ...
          {/* 이 학교가 속한 구간에 학교가 한둘뿐이면 막대가 거의 안 보인다.
              막대 높이는 그대로 두고, 라벨에서 막대까지 잇는 선으로 위치를 표시한다. */}
          <rect x={dist.binIndex * (bw + GAP) + bw / 2 - 1} y="0" width="2"
                height={Math.max(0, VB_H - Math.max(hereH, 2))} fill="#2166bd" opacity="0.45" />
        </svg>
